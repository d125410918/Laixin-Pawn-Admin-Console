...
function mapCustomer(row: Record<string, unknown>): Customer {
  const incomeRange = String(row.income_range ?? "");
  ...
  const incomeText = incomeRange
    ? `月入 ${incomeRange}`
    : incomeType === "daily"
      ? `日薪 ${dailyIncomeAmount || incomeAmount}`
      : `月入 ${incomeAmount}`;

  const fundingAmountWan = String(row.funding_amount_wan ?? "");
  const fundingNeedText = fundingAmountWan ? `${fundingAmountWan}萬` : String(row.funding_need ?? "");
